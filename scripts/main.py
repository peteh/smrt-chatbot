...
from igitur import GaudeamCalendar, GaudeamSession, GaudeamMembers
...
schema["article_summary"] = {
    "type": "dict",
    "schema": {"summary_bot": {"type": "string", "required": True}},
    "required": False,
}
...
def validate_config(config, schema) -> bool:
...
def run():
    ...
    main_pipe = pipeline.MainPipeline()
    ...
    if CONFIG_AI in configuration:
        ...
        for ai_config in ai_configs:
            if "ollama" in ai_config:
                ...
                bot_loader.add_bot(
                    ollama_conf["name"], QuestionBotOllama(ollama_server, model)
                )
                logging.info(
                    f"Registered Ollama '{ollama_conf['name']}' with model '{model}' at {ollama_server}"
                )
    ...
    if CONFIG_HOMEASSISTANT in configuration:
        ...
        ha_text_pipeline = pipeline.HomeassistantTextCommandPipeline(
            ha_token,
            ha_ws_api_url,
            chat_id_whitelist=ha_chat_id_whitelist,
            process_without_command=process_without_command,
        )
        ha_voice_pipeline = pipeline.HomeassistantVoiceCommandPipeline(
            ha_token, ha_ws_api_url, chat_id_whitelist=ha_chat_id_whitelist
        )
        ha_say_pipeline = pipeline.HomeassistantSayCommandPipeline(
            ha_token, ha_ws_api_url, chat_id_whitelist=ha_chat_id_whitelist
        )
        main_pipe.add_pipeline(ha_text_pipeline)
        main_pipe.add_pipeline(ha_voice_pipeline)
        main_pipe.add_pipeline(ha_say_pipeline)
    ...
    if CONFIG_GAUDEAM in configuration:
        ...
        for gaudeam_config in gaudeam_configs:
            gaudeam_email = gaudeam_config["user_email"]
            gaudeam_password = gaudeam_config.get("user_password")
            gaudeam_session = GaudeamSession.with_user_auth(gaudeam_email, gaudeam_password)
            ...
            gaudeam_pipeline = pipeline.GaudeamBdayPipeline(
                gaudeam_members, chat_id_whitelist, chat_id_blacklist
            )
            main_pipe.add_pipeline(gaudeam_pipeline)
            gaudeam_pipeline = pipeline.GaudeamCalendarPipeline(
                gaudeam_calendar, chat_id_whitelist, chat_id_blacklist
            )
            main_pipe.add_pipeline(gaudeam_pipeline)

            schedule_time = "09:00"
            # schedule daily birthday notifications
            bday_task = pipeline.GaudeamBdayScheduledTask(
                messenger_manager, chat_id_whitelist, gaudeam_members
            )

            schedule.every().day.at(schedule_time, "Europe/Berlin").do(bday_task.run)
            logging.info(
                f"Scheduled Gaudeam birthday notifications at {schedule_time} daily."
            )

            # schedule event notifications every day
            event_task = pipeline.GaudeamEventsScheduledTask(
                messenger_manager, chat_id_whitelist, gaudeam_calendar
            )
            schedule.every().day.at(schedule_time, "Europe/Berlin").do(event_task.run)
            logging.info(
                f"Scheduled Gaudeam event notifications at {schedule_time} daily."
            )
    ...
    if CONFIG_MESSAGE_GPT in configuration:
        ...
        message_answer_pipeline = pipeline.MessageQuestionPipeline(
            message_db,
            answer_bot,
        ...
        main_pipe.add_pipeline(message_answer_pipeline)
    ...
    if CONFIG_TTS in configuration:
        ...
        main_pipe.add_pipeline(tts_pipeline)
    ...
    if CONFIG_TINDER in configuration:
        ...
        tinder_pipeline = pipeline.TinderPipeline(
            tinder_bot, chat_id_whitelist, chat_id_blacklist
        )
        main_pipe.add_pipeline(tinder_pipeline)
    ...
    if CONFIG_ARTICLE_SUMMARY in configuration:
        ...
        article_summary_pipeline = pipeline.URLSummaryPipeline(
            article_summarizer, chat_id_whitelist, chat_id_blacklist
        )
        main_pipe.add_pipeline(article_summary_pipeline)
    ...
    if CONFIG_GALLERY in configuration:
        ...
        gallery_pipe = pipeline.GalleryPipeline(
            gallery_db, base_url, chat_id_whitelist, chat_id_blacklist
        )
        main_pipe.add_pipeline(gallery_pipe)
        gallery_delete_pipe = pipeline.GalleryDeletePipeline(
            gallery_db, chat_id_whitelist, chat_id_blacklist
        )
        main_pipe.add_pipeline(gallery_delete_pipe)
        ...
        if debug_flag:
            ...
            message_server_proc = Process(
                target=_serve_gallery, args=(gallery_port,), daemon=True
            )
            message_server_proc.start()
            logging.info(
                f"Started Gallery web server on port {gallery_port} (pid={message_server_proc.pid})"
            )
        else:
            from waitress import serve

            gallery_thread = threading.Thread(
                target=serve,
                args=(gallery_app.get_app(),),
                kwargs={"port": gallery_port},
                daemon=False,
            )
            gallery_thread.start()
            logging.info(
                f"Started Gallery web server on port {gallery_port} in thread."
            )

    CONFIG_CHATID = "chatid"
    if CONFIG_CHATID in configuration:
        chatid_pipeline = pipeline.ChatIdPipeline()
        main_pipe.add_pipeline(chatid_pipeline)
        lid_pipeline = pipeline.WhatsappLidPipeline()
        main_pipe.add_pipeline(lid_pipeline)
    ...
    if CONFIG_KLEINANZEIGEN in configuration:
        ...
        kleinanzeigen_task = pipeline.KleinanzeigenScheduledTask(
            keyword, messenger_manager, chat_id_whitelist
        )
        # run in background thread as we want to schedule internally
    ...
    if CONFIG_SIGNAL in configuration:
        ...
        signal_messenger = messenger.SignalMessenger(
            config_signal["number"], config_signal["host"], int(config_signal["port"])
        )
        messenger_manager.add_messenger(signal_messenger)
        signal_queue = messenger.SignalMessageQueue(signal_messenger, main_pipe.process)
    ...
    if CONFIG_TELEGRAM in configuration:
        ...
        telegram_messenger = messenger.TelegramMessenger(
            config_telegram["telegram_api_key"]
        )
        messenger_manager.add_messenger(telegram_messenger)
        telegram_queue = messenger.TelegramMessageQueue(
            telegram_messenger, main_pipe.process
        )
    ...
    if CONFIG_TELETHON in configuration:
        ...
        telethon_messenger = messenger.TelethonMessenger(
            config_telethon["api_id"],
            config_telethon["api_key"],
            config_telethon["bot_token"],
        )
        messenger_manager.add_messenger(telethon_messenger)
        telethon_queue = messenger.TelethonMessageQueue(
            telethon_messenger, main_pipe.process
        )
    ...
    if CONFIG_WHATSAPP in configuration:
        ...
        whatsapp_queue = messenger.WhatsappMessageQueue(whatsapp, main_pipe.process)
    ...
    if debug_flag:
        ...
        def _serve_message(message_server_port):
            ...
            message_server.run(
                host="0.0.0.0", port=message_server_port, debug=True, use_reloader=False
            )

        message_server_proc = Process(
            target=_serve_message, args=(message_server_port,), daemon=True
        )
        message_server_proc.start()
        logging.info(
            f"Started Message server on port {message_server_port} (pid={message_server_proc.pid})"
        )
    else:
        from waitress import serve
        ...
        gallery_thread = threading.Thread(
            target=serve,
            args=(message_server.get_app(),),
            kwargs={"port": message_server_port},
            daemon=False,
        )
        gallery_thread.start()
